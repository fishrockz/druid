...
fn main() {
    ...
    eprintln!("cargo run --example image --features \"images\"");
}
...
#[cfg(feature = "images")]
use druid::{
    widget::{Flex, Image, WidgetExt, ImageData},
...
fn ui_builder() -> impl Widget<u32> {
    let png_data = ImageData::from_str("examples/toppcbbling.png").unwrap();

    let mut col = Flex::column();

    col.add_child(Image::new(png_data.clone()).fix_width(100.0).center(), 1.0);
    col.add_child(Image::new(png_data), 1.0);
    col
}
